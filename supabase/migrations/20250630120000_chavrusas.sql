-- Chavrusa partner matching: listings + authenticated-only page visits.
...
-- Only signed-in users can browse listings; contact info never exposed to anon.
...
-- One visit row per user per calendar day (for unique visitor counts).
